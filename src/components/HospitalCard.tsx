...
import { MapPin, Droplet, Wind, Bed, Ambulance, AlertTriangle } from "lucide-react";
...
interface HospitalCardProps {
  ...
  onRequestClick: (hospital: any) => void;
}
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-background/50">
            ...
            {getStatusBadge(inventory.blood ?? 0)}
          </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-background/50">
            ...
            {getStatusBadge(inventory.oxygen ?? 0)}
          </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-background/50">
            ...
            {getStatusBadge(inventory.icu ?? 0)}
          </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-background/50">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-lg font-bold">
                  {inventory.ambulance ?? 0}
                </p>
              </div>
            </div>
            {getStatusBadge(inventory.ambulance ?? 0)}
          </div>
        </div>

        {/* Action */}
        <Button onClick={() => onRequestClick(hospital)} className="w-full">
          Request Resource
        </Button>
      </div>
    </Card>
